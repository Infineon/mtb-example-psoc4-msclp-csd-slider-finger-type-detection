 ...
 *              CSD Slider with Finger-type Detection Example for ModusToolbox.
 *
 *
 *
 * *******************************************************************************
...
/* Active mode Scan time updated for slider = finger type detection processing */
#define ACTIVE_MODE_FRAME_SCAN_TIME     (80u)

/* Active mode Processing time updated based on applicaion functionality */
#define ACTIVE_MODE_PROCESS_TIME        (95u)

/* ALR mode Scan time updated for slider application */
#define ALR_MODE_FRAME_SCAN_TIME        (80u)

/* ALR mode Processing time updated based on measured processing overhead */
#define ALR_MODE_PROCESS_TIME           (95u)
...
static void init_sys_tick(void);
static void start_runtime_measurement(void);
static uint32_t stop_runtime_measurement(void);
#endif
...
int main(void)
{
    ...
    #if ENABLE_RUN_TIME_MEASUREMENT
    static uint32_t active_scan_time = 0u;
    static uint32_t active_processing_time = 0u;
    #endif
    ...
    /* Initialize PWM block for LED2 */
    Cy_TCPWM_PWM_Init(tcpwm_0_cnt_0_HW, tcpwm_0_cnt_0_NUM, &tcpwm_0_cnt_0_config);
    /* Enable the initialized PWM */
    Cy_TCPWM_Enable_Multiple(tcpwm_0_cnt_0_HW, tcpwm_0_cnt_0_MASK);
    /* Then start the PWM */
    Cy_TCPWM_TriggerReloadOrIndex(tcpwm_0_cnt_0_HW, tcpwm_0_cnt_0_MASK);
        ...
        start_runtime_measurement();
            ...
            case ALR_MODE :
                ...
                if(Cy_CapSense_IsAnyWidgetActive(&cy_capsense_context))
                {
                    ...
                    /* Initialize PWM block for LED2 */
                    (void)Cy_TCPWM_PWM_Init(tcpwm_0_cnt_0_HW, tcpwm_0_cnt_0_NUM, &tcpwm_0_cnt_0_config);
                    /* Enable the initialized PWM */
                    Cy_TCPWM_Enable_Multiple(tcpwm_0_cnt_0_HW, tcpwm_0_cnt_0_MASK);
                    /* Then start the PWM */
                    Cy_TCPWM_TriggerReloadOrIndex(tcpwm_0_cnt_0_HW, tcpwm_0_cnt_0_MASK);
                ...
                else
                {
                    ...
                    if(TIMEOUT_RESET == capsense_state_timeout)
                    {
                      ...
                      break; /* End of Active-Low Refresh Rate(ALR) mode */
            ...
            case WOT_MODE :
                ...
                break; /* End of "WAKE_ON_TOUCH_MODE" */
 ...
 *******************************************************************************
 * Summary:
 *  Control LED2 in the kit to show finger type detection
 *  LED3 brightness varies based on touch position
 *******************************************************************************/
void led_control(void)
{
    static uint8_t led2_blink_state = 0u;
    uint8_t fingerType;
    cy_stc_capsense_touch_t *slider_touch_info;
    uint16_t slider_pos = 0u;

    fingerType = Cy_Capsense_GetFingerType(&cy_capsense_context, CY_CAPSENSE_LINEARSLIDER0_WDGT_ID);

    if ((fingerType == CY_CAPSENSE_FINGER_TYPE_NORMAL) ||
        (fingerType == CY_CAPSENSE_FINGER_TYPE_FAT) ||
        (fingerType == CY_CAPSENSE_FINGER_TYPE_LARGE_OBJECT))
    {
        /* Get slider status */
        slider_touch_info = Cy_CapSense_GetTouchInfo(CY_CAPSENSE_LINEARSLIDER0_WDGT_ID, &cy_capsense_context);

        if ((slider_touch_info != NULL) && (slider_touch_info->ptrPosition != NULL))
        {
    ...
    switch (fingerType)
    {
        case CY_CAPSENSE_FINGER_TYPE_NONE:
            /* Turn OFF LED2 and LED3 */
            Cy_TCPWM_PWM_SetCompare0(tcpwm_0_cnt_0_HW, tcpwm_0_cnt_0_NUM, 0u);
            Cy_TCPWM_PWM_SetCompare0(CYBSP_PWM_HW, CYBSP_PWM_NUM, 0u);
            break;

        case CY_CAPSENSE_FINGER_TYPE_NORMAL:
            /* LED2 OFF, LED3 brightness changes with position */
            Cy_TCPWM_PWM_SetCompare0(tcpwm_0_cnt_0_HW, tcpwm_0_cnt_0_NUM, 0u);
            Cy_TCPWM_PWM_SetCompare0(CYBSP_PWM_HW, CYBSP_PWM_NUM, (uint16_t)(slider_pos * 100u));
            break;

        case CY_CAPSENSE_FINGER_TYPE_FAT:
        {
            static uint8_t blink_divider = 0u;

            blink_divider++;
            if (blink_divider >= 20u)
            {
                blink_divider = 0u;
                led2_blink_state ^= 1u;
            }

            if (led2_blink_state != 0u)
            {
                Cy_TCPWM_PWM_SetCompare0(tcpwm_0_cnt_0_HW, tcpwm_0_cnt_0_NUM, 10000u);
            }
            else
            {
                Cy_TCPWM_PWM_SetCompare0(tcpwm_0_cnt_0_HW, tcpwm_0_cnt_0_NUM, 0u);
        ...
        case CY_CAPSENSE_FINGER_TYPE_LARGE_OBJECT:
            /* LED2 and LED3 ON at full brightness */
            Cy_TCPWM_PWM_SetCompare0(tcpwm_0_cnt_0_HW, tcpwm_0_cnt_0_NUM, 10000u);
            Cy_TCPWM_PWM_SetCompare0(CYBSP_PWM_HW, CYBSP_PWM_NUM, 10000u);
            break;
